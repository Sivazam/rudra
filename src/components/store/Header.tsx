...
import { 
  Menu, Search, Heart, ShoppingCart, User, Package, LogOut, 
  Phone, MapPin, Edit, AlertCircle, ShoppingBag 
} from 'lucide-react';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { Sheet, SheetContent, SheetTrigger } from '@/components/ui/sheet';
...
import Link from 'next/link';
import { isUserAuthenticated, getCurrentUser } from '@/lib/auth';
import { isProfileComplete } from '@/lib/profileCompletionMiddleware';
import { useRouter } from 'next/navigation';
import { wishlistService } from '@/lib/services/wishlistService';
      ...
      if (authStatus) {
        ...
        if (userToken) {
          try {
            const { userService } = await import('@/lib/services');
            const fullUser = await userService.getUserByPhoneNumber(userToken.phoneNumber);
            setCurrentUser(fullUser || userToken);
          } catch (error) {
            console.error('Error fetching user profile:', error);
        ...
        <div className="flex items-center justify-between h-16">
          ...
          <Link href="/" className="flex items-center justify-center sm:justify-start space-x-2">
            ...
            <div className="leading-tight text-center sm:text-left">
              <span className="block text-lg sm:text-xl font-bold font-spiritual" style={{ color: '#755e3e' }}>
                Sanathan
              </span>
              <span className="block text-base sm:text-xl font-bold font-spiritual" style={{ color: '#755e3e' }}>
                Rudraksha
              </span>
            </div>
          </Link>
